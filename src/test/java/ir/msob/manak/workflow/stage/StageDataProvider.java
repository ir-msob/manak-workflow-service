...
/**
 * This class provides test data for the {@link Stage} class. It extends the {@link DomainCrudDataProvider} class
 * and provides methods to create new test data objects, update existing data objects, and generate JSON patches for updates.
 */
@Component
public class StageDataProvider extends DomainCrudDataProvider<Stage, StageDto, StageCriteria, StageRepository, StageService> {
     ...
     * Creates a new DTO object with mandatory fields set.
     */
    public static StageDto prepareMandatoryDto() {
    ...
    public JsonPatch getJsonPatch() {
        ...
        operations.add(new ReplaceOperation(new JsonPointer(String.format("/%s", Stage.FN.description)), new TextNode(UPDATED_STRING)));
     ...
     */
    @Override
    public void updateDto(StageDto dto) {
     ...
     */
    @Override
    public StageDto getMandatoryNewDto() {
     ...
     */
    @Override
    public void updateMandatoryDto(StageDto dto) {
    ...
    public List<JsonPatchOperation> getMandatoryJsonPatchOperation() throws JsonPointerException {
        ...
        operations.add(new ReplaceOperation(new JsonPointer(String.format("/%s", Stage.FN.name)), new TextNode(UPDATED_STRING)));
        return operations;
    }

    @Override
    public void assertMandatoryGet(StageDto before, StageDto after) {
        super.assertMandatoryGet(before, after);
        Assertions.assertThat(after.getName()).isEqualTo(before.getName());
    }

    @Override
    public void assertGet(StageDto before, StageDto after) {
    ...
    @Override
    public void assertMandatoryUpdate(StageDto dto, StageDto updatedDto) {
    ...
    @Override
    public void assertUpdate(StageDto dto, StageDto updatedDto) {
        super.assertUpdate(dto, updatedDto);
        assertMandatoryUpdate(dto, updatedDto);
    }

    @Override
    public void assertMandatorySave(StageDto dto, StageDto savedDto) {
        super.assertMandatorySave(dto, savedDto);
        assertMandatoryGet(dto, savedDto);
    }

    @Override
    public void assertSave(StageDto dto, StageDto savedDto) {
